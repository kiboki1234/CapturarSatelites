from pathlib import Path
import numpy as np


# ============================================================
# RUTAS
# ============================================================

ROOT = Path(__file__).resolve().parent.parent

INPUT_DIR = (
    ROOT
    / "outputs"
    / "decoded"
    / "viterbi"
)

OUTPUT_DIR = (
    ROOT
    / "outputs"
    / "decoded"
    / "derandomized"
)

OUTPUT_DIR.mkdir(
    parents=True,
    exist_ok=True
)


# ============================================================
# ASM CCSDS
# ============================================================

ASM = np.array(
    [0x1D, 0xCF, 0xFC, 0x1D],
    dtype=np.uint8
)


# ============================================================
# SECUENCIA PN CCSDS
#
# Exactamente la secuencia utilizada por SatDump.
# Tiene periodo de 255 bytes.
# ============================================================

CCSDS_PN = np.array([
    
], dtype=np.uint8)


# ============================================================
# DERANDOMIZACION
# ============================================================

def derandomize(data):

    result = data.copy()

    # SatDump aplica derand a frame[4:]
    for i in range(4, len(result)):

        result[i] ^= CCSDS_PN[
            (i - 4) % 255
        ]

    # Como SatDump, ponemos ASM limpio/canónico
    result[:4] = ASM

    return result


# ============================================================
# FORMATO HEX
# ============================================================

def hex_string(data, n=32):

    return " ".join(
        f"{int(x):02X}"
        for x in data[:n]
    )


# ============================================================
# PROCESAR
# ============================================================

print()
print("================================================")
print("        DERANDOMIZACION CCSDS / LRPT")
print("================================================")
print()


for frame_file in sorted(
    INPUT_DIR.glob("frame_*.bin")
):

    raw = np.fromfile(
        frame_file,
        dtype=np.uint8
    )

    if len(raw) != 1024:

        print(
            f"{frame_file.name}: tamaño incorrecto "
            f"({len(raw)} bytes)"
        )

        continue


    decoded = derandomize(
        raw
    )


    # --------------------------------------------------------
    # Los tres bytes después de los dos primeros bytes
    # muestran un contador muy útil para comprobar continuidad.
    # --------------------------------------------------------

    counter_candidate = int.from_bytes(
        decoded[6:9].tobytes(),
        byteorder="big"
    )


    print(
        "------------------------------------------------"
    )

    print(
        frame_file.name
    )

    print()

    print(
        "ANTES:"
    )

    print(
        hex_string(raw)
    )

    print()

    print(
        "DESPUES:"
    )

    print(
        hex_string(decoded)
    )

    print()

    print(
        f"Primeros bytes después del ASM : "
        f"{hex_string(decoded[4:], 12)}"
    )

    print(
        f"Contador 24-bit candidato      : "
        f"0x{counter_candidate:06X} "
        f"({counter_candidate})"
    )


    salida = (
        OUTPUT_DIR
        / frame_file.name.replace(
            "frame_",
            "derand_"
        )
    )

    decoded.tofile(
        salida
    )

    print(
        f"Guardado                       : {salida}"
    )

    print()


print("================================================")
print("FIN")
print("================================================")